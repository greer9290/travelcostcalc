...
from src.gui import settings_tab, simple_calendar
...
class TravelCostApp(ttk.Window):
    ...
    # Clear existing data
        ...
        try:
            with open(config.json_flight_path, 'r') as f:
                flight_data = json.load(f)
        except FileNotFoundError:
            print("Flight data file not found.")
            print(config.json_flight_path)
            return
    ...
    def create_widgets(self):
        ...
        left_frame.pack(side=tk.FIRST, fill=tk.BOTH, expand=True, padx=(0, 5))
        ...
        if len(self.tree.get_children()) == 0:
            ...
            print(f"Treeview loaded with {len(self.tree.get_children())} items")
